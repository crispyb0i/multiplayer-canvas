...
describe("document model", () => {
  it("applies immutable add, update, and remove commands", () => {
    ...
    const moved = applyCommand(withShape, {
      type: "update",
      id: rectangle.id,
      changes: { x: 40 },
    });
  ...
  it("rejects malformed or unsafe data", () => {
    ...
    expect(() =>
      deserializeDocument(
        JSON.stringify({ version: 1, shapes: [{ ...rectangle, width: -1 }] }),
      ),
    ).toThrow(/dimensions cannot be negative/);
    expect(() => createDocument([rectangle, rectangle])).toThrow(
      /Duplicate shape id/,
    );
  });
  ...
  it("rejects missing and duplicate command targets", () => {
    const document = createDocument([rectangle]);
    expect(() =>
      applyCommand(document, { type: "add", shape: rectangle }),
    ).toThrow(/already exists/);
    expect(() =>
      applyCommand(document, { type: "remove", id: "missing" }),
